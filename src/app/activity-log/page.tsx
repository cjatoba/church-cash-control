...
export default async function ActivityLogPage() {
  ...
  return (
    <div className="flex flex-1 items-center justify-center bg-zinc-50 dark:bg-black py-10">
      <div className="flex w-full max-w-lg flex-col gap-4 rounded-lg border border-black/[.08] bg-white p-8 dark:border-white/[.16] dark:bg-zinc-950">
        <BackLink href="/" />
        ...
        {entries.length === 0 ? (
          ...
          <ul className="flex flex-col gap-2">
            {entries.map((entry, index) => (
              <li
                key={index}
                className="flex flex-col gap-0.5 rounded border border-black/[.08] px-3 py-3 text-sm dark:border-white/[.16]"
              >
